document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchAndRenderPedidos() {
        ...
        try {
            ...
            pedidos.forEach(pedido => {
                ...
                const realizarVentaButtonDisabled = (accionesDeshabilitadas || !tieneStock) ? 'disabled' : '';
                const realizarVentaButtonClass = (accionesDeshabilitadas || !tieneStock) ? 'btn-secondary' : 'btn-success';
                ...
                // Creamos la cadena del producto con la marca y el peso
                 const productoInfo = `${pedido.nombreMarca} ${pedido.nombreProducto} (${pedido.peso} kg)`;
                    ...
                    row.innerHTML = `
                        ...
                        <td>${productoInfo}</td> <td>${pedido.unidadesSolicitadas}</td>
                        ...
                        <td>
                            ...
                            <button class="btn btn-sm ${realizarVentaButtonClass} sale-btn" ${realizarVentaButtonDisabled} data-id="${pedido.idPedidoPendiente}">
    ...
    // Manejador del evento de "Realizar Venta" (usa delegación de eventos)
    document.querySelector('#datatablesSimple tbody').addEventListener('click', async (event) => {
        if (event.target.closest('.sale-btn')) {
            const button = event.target.closest('.sale-btn');
            const id = button.getAttribute('data-id');

            const response = await authenticatedFetch(`${baseUrl}/pedidoPendiente/${id}/marcar-resuelto`, {
                method: 'PATCH'
            });
